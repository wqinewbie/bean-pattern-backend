...
public interface GiftItemMapper {
    ...
    @Select("SELECT id, gift_type_id AS giftTypeId, gift_code AS giftCode, name, value, " +
            ...
            "FROM bp_gift_item WHERE gift_type_id = #{giftTypeId} AND status = 1 " +
    ...
    @Select("SELECT id, gift_type_id AS giftTypeId, gift_code AS giftCode, name, value, " +
            ...
            "start_at AS startAt, end_at AS endAt, sort_order AS sortOrder, status, " +
            "created_at AS createdAt, updated_at AS updatedAt " +
            "FROM bp_gift_item WHERE gift_code = #{giftCode}")
    ...
    @Select("SELECT id, gift_type_id AS giftTypeId, gift_code AS giftCode, name, value, " +
            ...
            "start_at AS startAt, end_at AS endAt, sort_order AS sortOrder, status, " +
            "created_at AS createdAt, updated_at AS updatedAt " +
            "FROM bp_gift_item WHERE status = 1 " +
            "AND (start_at IS NULL OR start_at <= #{now}) " +
            "AND (end_at IS NULL OR end_at >= #{now}) " +
            "ORDER BY sort_order ASC")
    List<GiftItem> findAllAvailable(@Param("now") LocalDateTime now);

    @Update("UPDATE bp_gift_item SET remain_quantity = remain_quantity - 1 WHERE id = #{id} AND remain_quantity > 0")
    int decrementQuantity(@Param("id") Long id);

    @Update("UPDATE bp_gift_item SET remain_quantity = remain_quantity + 1 WHERE id = #{id} AND total_quantity > 0")
