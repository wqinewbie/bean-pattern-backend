...
public interface PopupConfigMapper {
    ...
    @Select("SELECT id, `key`, title, content, image_url, button_text, button_url, priority, enabled, start_time, end_time, show_interval, created_at, updated_at FROM bp_popup_config WHERE enabled = 1 AND (start_time IS NULL OR start_time <= NOW()) AND (end_time IS NULL OR end_time >= NOW()) ORDER BY priority DESC")
    List<PopupConfig> findActive();

    @Select("SELECT id, `key`, title, content, image_url, button_text, button_url, priority, enabled, start_time, end_time, show_interval, created_at, updated_at FROM bp_popup_config ORDER BY priority DESC")
    List<PopupConfig> findAll();

    @Select("SELECT id, `key`, title, content, image_url, button_text, button_url, priority, enabled, start_time, end_time, show_interval, created_at, updated_at FROM bp_popup_config WHERE `key` = #{key}")
