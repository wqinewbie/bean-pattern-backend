...
public interface UserVipRecordMapper {
    ...
    @Select("SELECT id, user_id AS userId, product_id AS productId, product_code AS productCode, " +
            ...
            "FROM user_vip_record WHERE user_id = #{userId} AND status = 1 " +
    ...
    @Select("SELECT id, user_id AS userId, product_id AS productId, product_code AS productCode, " +
            ...
            "start_at AS startAt, expire_at AS expireAt, ai_used_count AS aiUsedCount, " +
            "ai_reset_at AS aiResetAt, status, created_at AS createdAt, updated_at AS updatedAt " +
            "FROM user_vip_record WHERE user_id = #{userId} ORDER BY created_at DESC")
    List<UserVipRecord> findByUserId(@Param("userId") Long userId);

    @Insert("INSERT INTO user_vip_record(user_id, product_id, product_code, vip_level, order_id, order_no, " +
    ...
    @Update("UPDATE user_vip_record SET ai_used_count = ai_used_count + 1 WHERE id = #{id}")
    int incrementAiUsedCount(@Param("id") Long id);

    @Update("UPDATE user_vip_record SET status = 0, updated_at = CURRENT_TIMESTAMP WHERE user_id = #{userId} AND status = 1")
    int deactivateAll(@Param("userId") Long userId);

    @Update("UPDATE user_vip_record SET ai_used_count = 0, updated_at = CURRENT_TIMESTAMP WHERE id = #{id}")
    int resetAiUsedCount(@Param("id") Long id);

    @Update("UPDATE user_vip_record SET ai_reset_at = #{resetAt}, updated_at = CURRENT_TIMESTAMP WHERE id = #{id}")
